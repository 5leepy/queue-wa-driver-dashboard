...
// =============================================================================
// Pure helpers (outside component so sub-components can use them)
// =============================================================================

function getDurationText(joinedAtStr) {
...
function formatJoinTime(dateStr) {
  if (!dateStr) return '';
  const date = new Date(dateStr);
  return date.toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' });
}

function formatHullNumber(val) {
  if (!val) return '';
  ...
  if (isNaN(originalNum) || originalNum <= 0 || originalNum > 1500) return null;
...
export default function Home() {
  ...
  const pinnedHullRef = useRef(pinnedHull);
  const poisRef = useRef(pois);

  // Sync refs
  useEffect(() => { activePoiIdRef.current = activePoiId; }, [activePoiId]);
  useEffect(() => { pinnedHullRef.current = pinnedHull; }, [pinnedHull]);
  useEffect(() => { poisRef.current = pois; }, [pois]);

  // ---------------------------------------------------------------------------
  // Track action helper - dual GA4 + Vercel Analytics
  // ---------------------------------------------------------------------------
  const trackAction = useCallback((eventName, params = {}) => {
    const enrichedParams = { ...params, driver_hull: pinnedHullRef.current || 'unknown' };
    try { track(eventName, enrichedParams); } catch (err) { console.warn('Vercel track error:', err); }
    try {
      if (typeof window !== 'undefined' && window.gtag) window.gtag('event', eventName, enrichedParams);
    } catch (err) { console.warn('GA track error:', err); }
  }, []);

  // ---------------------------------------------------------------------------
  // Load pinned hull from localStorage on mount
  // ---------------------------------------------------------------------------
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      if (!stored && !skipped) setShowOnboarding(true);
    }
  }, []);
  ...
  // ---------------------------------------------------------------------------
  const hullValidationStatus = (() => {
    if (!tempHullInput) return null;
    const formatted = formatHullNumber(tempHullInput);
    return formatted ? { ok: true, formatted } : { ok: false };
  })();
  ...
  const handleSaveHullInput = (val) => {
    ...
    if (formatted) {
      ...
      setTempHullInput(pinnedHull); // revert
    }
  };
  ...
  const handleOnboardingSave = () => {
    ...
    if (!val) { setOnboardingError('Nomor lambung tidak boleh kosong.'); return; }
  ...
  const handleOnboardingSkip = () => {
    ...
    if (typeof window !== 'undefined') localStorage.setItem('skipOnboarding', 'true');
    setShowOnboarding(false);
  };
  ...
  const getWsUrl = () => {
    if (typeof window !== 'undefined') {
      const h = window.location.hostname;
      if (h === 'localhost' || h === '127.0.0.1') return 'ws://localhost:5051';
    }
  ...
  // ---------------------------------------------------------------------------
  // Scan driver position across all loaded POIs using queue endpoint
  // Called when pinnedHull changes or pois list changes
  // ---------------------------------------------------------------------------
  const scanAllDriverPositions = useCallback(async (currentPois, hull) => {
    if (!hull || currentPois.length === 0) { setDriverPositions({}); return; }
    const apiUrl = getApiUrl();
    const normalizedPin = hull.trim().toLowerCase();
    const results = await Promise.allSettled(
      currentPois.map(async (poi) => {
        const res = await fetch(`${apiUrl}/api/pois/${poi.id}/queues`);
        if (!res.ok) throw new Error();
        const data = await res.json();
        return { poiId: poi.id, data };
      })
    );
    const newPositions = {};
    results.forEach((result) => {
      if (result.status !== 'fulfilled') return;
      const { poiId, data } = result.value;
      const standbyIdx = (data.standby || []).findIndex(
        item => item.hullNumber.trim().toLowerCase() === normalizedPin
      );
      if (standbyIdx !== -1) { newPositions[poiId] = { pos: standbyIdx + 1, type: 'standby' }; return; }
      const tandonIdx = (data.tandon || []).findIndex(
        item => item.hullNumber.trim().toLowerCase() === normalizedPin
      );
      if (tandonIdx !== -1) { newPositions[poiId] = { pos: tandonIdx + 1, type: 'tandon' }; return; }
      newPositions[poiId] = null;
    });
    setDriverPositions(newPositions);
  }, []);

  // Re-scan whenever pinnedHull or pois list changes
  useEffect(() => {
    if (pois.length > 0) scanAllDriverPositions(pois, pinnedHull);
  }, [pinnedHull, pois, scanAllDriverPositions]);

  // Also update single-POI position from already-loaded activePoiQueues data (faster, no extra fetch)
  useEffect(() => {
    if (!activePoiQueues || !pinnedHull) return;
    const poiId = activePoiQueues.poi.id;
    const normalizedPin = pinnedHull.trim().toLowerCase();
    const standbyIdx = (activePoiQueues.standby || []).findIndex(
      item => item.hullNumber.trim().toLowerCase() === normalizedPin
    );
    if (standbyIdx !== -1) {
      setDriverPositions(prev => ({ ...prev, [poiId]: { pos: standbyIdx + 1, type: 'standby' } }));
      return;
    }
    const tandonIdx = (activePoiQueues.tandon || []).findIndex(
      item => item.hullNumber.trim().toLowerCase() === normalizedPin
    );
    if (tandonIdx !== -1) {
      setDriverPositions(prev => ({ ...prev, [poiId]: { pos: tandonIdx + 1, type: 'tandon' } }));
      return;
    }
    setDriverPositions(prev => ({ ...prev, [poiId]: null }));
  ...
  const fetchPoiQueues = useCallback(async (poiId) => {
    ...
    try {
      ...
      setLastQueueUpdated(formatCurrentTime());
    } catch (err) {
  ...
  const connectWS = useCallback(() => {
    ...
    ws.onopen = () => {
      ...
      fetchPois();
      if (activePoiIdRef.current) fetchPoiQueues(activePoiIdRef.current);
    };
    ...
    ws.onmessage = (event) => {
      try {
        ...
        if (message.type === 'QUEUE_UPDATE') {
          ...
          // Update driver position for the changed POI if driver hull is set
          if (pinnedHullRef.current && message.poiId) {
            const apiUrl = getApiUrl();
            const normalizedPin = pinnedHullRef.current.trim().toLowerCase();
            fetch(`${apiUrl}/api/pois/${message.poiId}/queues`)
              ...
              .then(data => {
                ...
                if (standbyIdx !== -1) {
                  setDriverPositions(prev => ({ ...prev, [message.poiId]: { pos: standbyIdx + 1, type: 'standby' } }));
                  return;
                }
                const tandonIdx = (data.tandon || []).findIndex(
                  item => item.hullNumber.trim().toLowerCase() === normalizedPin
                ...
                if (tandonIdx !== -1) {
                  setDriverPositions(prev => ({ ...prev, [message.poiId]: { pos: tandonIdx + 1, type: 'tandon' } }));
                  return;
                }
                setDriverPositions(prev => ({ ...prev, [message.poiId]: null }));
              })
              .catch(() => {});
          }
        }
  ...
  const requestWakeLock = async () => {
    if (typeof window !== 'undefined' && 'wakeLock' in navigator) {
      try { wakeLockRef.current = await navigator.wakeLock.request('screen'); } catch (err) { /* silent */ }
    }
  };
  ...
  // ---------------------------------------------------------------------------
  // Track position change → trigger beep
  // ---------------------------------------------------------------------------
  useEffect(() => {
    ...
    const poiId = activePoiQueues.poi.id;
    const normalizedPin = pinnedHull.trim().toLowerCase();
    if (!normalizedPin) { prevPosRef.current = { poiId: null, position: null, status: null }; return; }

    let newPos = null, newStatus = null;
    const standbyIndex = (activePoiQueues.standby || []).findIndex(
      item => item.hullNumber.trim().toLowerCase() === normalizedPin
    );
    if (standbyIndex !== -1) { newPos = standbyIndex + 1; newStatus = 'standby'; }
    else {
      const tandonIndex = (activePoiQueues.tandon || []).findIndex(
        item => item.hullNumber.trim().toLowerCase() === normalizedPin
      );
      if (tandonIndex !== -1) { newPos = tandonIndex + 1; newStatus = 'tandon'; }
    }
    ...
    if (prev.poiId === poiId && prev.position !== null && newPos !== null) {
      ...
      if (movedUpInStatus || movedUpInPosition) playBeep();
    }
    prevPosRef.current = { poiId, position: newPos, status: newStatus };
  }, [activePoiQueues, pinnedHull]);

  // ---------------------------------------------------------------------------
  // Initial Load, WebSocket setup, Screen Wake Lock, Service Worker
  // ---------------------------------------------------------------------------
  useEffect(() => {
    fetchPois();
    connectWS();
  ...
  }, [fetchPois, connectWS, fetchPoiQueues]);
  ...
  useEffect(() => {
    if (!error) return;
    const interval = setInterval(() => fetchPois(false), 15000);
    return () => clearInterval(interval);
  }, [error, fetchPois]);

  // ---------------------------------------------------------------------------
  // Periodic fallback polling every 60s even when connected (data freshness)
  // ---------------------------------------------------------------------------
  useEffect(() => {
  ...
  const getWibHour = () => {
    ...
    return new Date(utc + 3600000 * 7).getHours();
  };

  const isPoiOpen = (poi) => {
    const h = getWibHour();
    return h >= poi.onlineHour && h < poi.offlineHour;
  };
  ...
  const getPoiLocation = (code) => {
    const locations = { PTC: 'Surabaya Barat', PCM: 'Surabaya Timur', RP: 'Surabaya Selatan', TP: 'Surabaya Pusat' };
  ...
  const handlePoiClick = (poiId) => {
    ...
    if (clickedPoi) trackAction('view_pangkalan_detail', { poi_id: poiId, poi_code: clickedPoi.code, poi_name: clickedPoi.name });
  ...
  const handleManualRefresh = () => {
    ...
    trackAction('click_manual_refresh', { active_poi_code: activePoi?.code || 'none' });
    fetchPois(true);
    if (activePoiId) fetchPoiQueues(activePoiId);
  };

  const handleDetailRefresh = () => {
    if (loadingQueues) return;
    trackAction('click_detail_refresh', { poi_code: activePoi?.code });
    if (activePoiId) fetchPoiQueues(activePoiId);
    fetchPois(false);
  };
  ...
  return (
    <div className="app-container">
      ...
      <header className="app-header">
        ...
        <div className="header-actions">
          <button
            ...
            onClick={() => { trackAction('click_help_guide'); setShowGuide(true); }}
      ...
      <main className="app-main">
        {loadingPois && pois.length === 0 ? (
          ...
          <div className="offline-container">
            ...
            <p className="offline-desc">Gagal menghubungkan ke server antrean. Pastikan perangkat Anda terhubung ke internet atau coba lagi beberapa saat lagi.</p>
            <button className="offline-retry-btn" onClick={() => { trackAction('click_offline_retry'); fetchPois(false); }} disabled={isRefreshing}>
              {isRefreshing ? (<><span className="spinner-mini"></span>Menghubungkan...</>) : 'Coba Hubungkan Kembali'}
            </button>
                    ...
                    <div className="poi-card-header">
                      ...
                      <div className="poi-details-block">
                        <div className="poi-title-row">
                          ...
                          <span className={`status-badge ${open ? 'buka' : 'tutup'}`}>{open ? 'BUKA' : 'TUTUP'}</span>
                        </div>
                    ...
                    <div className="poi-progress-row">
                      <div className="progress-col">
                        ...
                        <div className="progress-bar-container">
                          <div className="progress-bar-fill standby" style={{ width: `${standbyPercentage}%` }}></div>
                        </div>
                      </div>
                      <div className="progress-col">
                        <div className="progress-header">
                        ...
                        <div className="progress-bar-container">
                          <div className="progress-bar-fill tandon" style={{ width: `${tandonPercentage}%` }}></div>
                        </div>
                      </div>
                    </div>
      ...
      {!loadingPois && pois.length > 0 && (
        <footer className="app-footer">
          <div className="footer-top-row">
            ...
            <button className="refresh-button" onClick={handleManualRefresh} disabled={isRefreshing}>
        ...
        <div className="detail-view">
          <header className="detail-header">
            ...
            <div className="detail-header-info">
              ...
              <span className="detail-poi-status">
                <span className="status-indicator-dot" style={{ backgroundColor: isPoiOpen(activePoi) ? 'var(--color-success)' : 'var(--color-danger)' }}></span>
                {isPoiOpen(activePoi) ? 'Buka Sekarang' : 'Sedang Tutup'} • {formatHour(activePoi.onlineHour)}-{formatHour(activePoi.offlineHour)} WIB
              </span>
            </div>
            {/* Manual refresh button in detail header */}
            <button
            ...
            >
              <svg className={`sync-icon ${loadingQueues ? 'spinning' : ''}`} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" style={{ width: 16, height: 16 }}>
                <path d="M21.5 2v6h-6M21.34 15.57a10 10 0 1 1-.57-8.38l5.67-5.67" />
              </svg>
            </button>
          </header>
          ...
          <div className="detail-summary-panel">
            <div className="summary-box">
              ...
              <span className="summary-val">{activePoi.standbyCount}/{activePoi.maxStandby}</span>
            </div>
            <div className="summary-box">
              <span className="summary-label">TANDON</span>
              <span className="summary-val">{activePoi.tandonCount}/{activePoi.maxTandon}</span>
            </div>
            <div className="summary-box">
          ...
          {/* Last updated timestamp for detail view */}
          {lastQueueUpdated && (
            <div className="detail-last-updated">
              <span>Data antrean: {lastQueueUpdated}</span>
            </div>
          )}
              ...
              <>
                ...
                <div className="detail-section">
                  <div className="section-header standby-header">
                    ...
                    <span className="section-count">{activePoiQueues?.standby?.length || 0} / {activePoi.maxStandby}</span>
                  </div>
                ...
                <div className="detail-section">
                  <div className="section-header tandon-header">
                    ...
                    <span className="section-count">{activePoiQueues?.tandon?.length || 0} / {activePoi.maxTandon}</span>
                  </div>
      ...
      {showOnboarding && (
        <div className="onboarding-overlay">
          <div className="onboarding-card">
            ...
            <p className="onboarding-desc">Masukkan nomor lambung untuk menyorot posisi antrean Anda secara otomatis dan mengaktifkan peringatan suara (beep) ketika antrean Anda naik.</p>
            <div className="onboarding-input-wrapper">
              <input
                ...
                onChange={(e) => { setOnboardingInput(e.target.value); setOnboardingError(''); }}
            ...
            <div className="onboarding-buttons">
              <button className="onboarding-btn-skip" onClick={handleOnboardingSkip}>Lewati</button>
              <button className="onboarding-btn-save" onClick={handleOnboardingSave}>Simpan</button>
            </div>
          </div>
        </div>
      )}
      ...
      {/* GUIDE MODAL OVERLAY */}
      {showGuide && (
        <div className="guide-overlay" onClick={() => { trackAction('close_help_guide', { method: 'overlay_click' }); setShowGuide(false); }}>
          <div className="guide-card" onClick={(e) => e.stopPropagation()}>
            <div className="guide-header">
              <h3 className="guide-title">ℹ️ Panduan Antrean WhatsApp</h3>
              <button className="guide-close-btn" onClick={() => { trackAction('close_help_guide', { method: 'close_button' }); setShowGuide(false); }}>×</button>
            </div>
            <div className="guide-body">
              <p className="guide-subtitle-text">Kirim pesan dengan format berikut ke nomor WhatsApp bot koordinator untuk memperbarui antrean Anda:</p>
              <ul className="guide-list">
                <li><span className="guide-cmd"><code>[No_Lambung] in</code></span><span className="guide-desc">Masuk antrean (contoh: <strong>006 in</strong>)</span></li>
                <li><span className="guide-cmd"><code>[No_Lambung] out</code></span><span className="guide-desc">Keluar dari antrean (contoh: <strong>006 out</strong>)</span></li>
                <li><span className="guide-cmd"><code>[No_Lambung] pos</code></span><span className="guide-desc">Cek urutan posisi antrean Anda (contoh: <strong>006 pos</strong>)</span></li>
                <li><span className="guide-cmd"><code>[No_Lambung] off</code></span><span className="guide-desc">Istirahat / Ngopi (contoh: <strong>006 off</strong> - maksimal 45 menit, jatah 1x)</span></li>
                <li><span className="guide-cmd"><code>[No_Lambung] on</code></span><span className="guide-desc">Selesai istirahat / aktif kembali (contoh: <strong>006 on</strong>)</span></li>
              </ul>
              <div className="guide-footer-note">*Harap pastikan nomor WhatsApp Anda sudah terdaftar di sistem koordinator.</div>
            </div>
            <button className="guide-done-btn" onClick={() => { trackAction('close_help_guide', { method: 'understand_button' }); setShowGuide(false); }}>Mengerti</button>
          </div>
        </div>
      )}
    </div>
